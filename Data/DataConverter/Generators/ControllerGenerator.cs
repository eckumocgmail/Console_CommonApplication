...
namespace DataConverter.Generators
{
    ...
    {
        public ControllerGenerator(IServiceProvider models) : base(models)
        {
        }
        ...
        public Dictionary<string, string> GetJsApi()
        {
            ...
            foreach (Type controllerType in controllers)
            {
                ...
                try
                {
                    api[controllerType.GetNameOfType()] = GenerateAngularJsService(controllerType);
                }catch (Exception ex)
                {
        ...
        public string AngularJsService(MyControllerModel controllerModel)
        {
            ...
            foreach (var actionKV in controllerModel.Actions)
            {
                ...
                typeScript += "\t\t\t\treturn window['https']({ url: '" + actionModel.Path+ "', type: '"+ TextNaming.ToCapitalStyle(controllerModel.Name).Replace("Controller", "Factory") + "', params: pars, headers: { 'Content-Type': 'text/json', Scope: angular.element(pelement).scope().$id, Model: pelement?pelement.id.replace('view-',''): '' }, method: '" + actionModel.Method+"',}).then((r)=>{ window['checkout'](); return r; });\n";
                typeScript += "\t\t\t}\n\n\n";
            }
            typeScript+="}\n\n\n";
            ...
            typeScript += "@Controller({ name: '"+TextNaming.ToCapitalStyle(controllerModel.Name )+ "' })\n";
            ...
            typeScript += "\n\t    $scope.$element=$element; this.ctrlName = '"+ TextNaming.ToCapitalStyle(controllerModel.Name).Replace("Controller", "Factory") + "';\n";
            typeScript += "\n\t    $element[0].ctrl=$scope; const ctrl = this; window['app'].scopes[$scope.$id]=this;\n";
            typeScript += "\n\t    $scope.$watch('model',function(e){ console.log(e); })\n";

            typeScript += "\n\t    Object.assign($scope,new " + TextNaming.ToCapitalStyle(controllerModel.Name).Replace("Controller", "Factory") + "($element[0])); \n";
            if (TypeUtils.HasBaseType(ReflectionService.TypeForName(controllerModel.Name), typeof(BaseController )))
            {
            ...
            typeScript += "\n\t    window['$" + TextNaming.ToCamelStyle(controllerModel.Name).Replace("Controller","") + "'] = $scope; \n";
            typeScript += "\n\t    window['$" + TextNaming.ToCamelStyle(controllerModel.Name) + "'] = $scope; \n";
            typeScript += "\n\t    //console.log('$" + TextNaming.ToCamelStyle(controllerModel.Name) + "', window['$" + TextNaming.ToCamelStyle(controllerModel.Name) + "'] = $scope); \n";
            typeScript += "\n\t    $scope['$" + TextNaming.ToCamelStyle(controllerModel.Name) + "']=window['$" + TextNaming.ToCamelStyle(controllerModel.Name) + "'] = $scope; \n";
            typeScript +=
            "\n\n\t\tfunction toHttpParams(obj: any): {[property: string]: string} " +
            "\n\t\t{ " +
            "\n\t\t     const result: {[property: string]: string} = { }; " +
