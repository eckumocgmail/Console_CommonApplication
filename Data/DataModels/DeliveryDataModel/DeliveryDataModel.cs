...
namespace DataEntities
{
            ...
            where THolder : ProductHolder<TItem>
    {
        ...
        public dynamic GetDataSet(string name)
        {
            if (typeof(TCustomer).GetNameOfType().Equals(name)) return this.Customers;
            if (typeof(THolder).GetNameOfType().Equals(name)) return this.Holders;
            switch (name)
            {
                ...
                // case nameof(ProductsInStock): return this.ProductsInStock;
    ...
    */
    ...
    /*public static void ConfigureDeliveryServices(IServiceCollection services, IConfiguration config)
    {
        ...
        services.AddScoped(
            typeof(IUnitOfWork<ProductCustomer<TItem>>),
            sp => new UnitOfWork<Order<TItem>>(
                sp.GetService<DeliveryDbContext<TCustomer, TItem, THolder>>()));
        services.AddScoped(
            typeof(IUnitOfWork<Holder<TItem>>),
            sp => new UnitOfWork<Holder<TItem>>(
                sp.GetService<DeliveryDbContext<TCustomer, TItem, THolder>>()));
        services.AddScoped(typeof(IUnitOfWork<Product>), sp => new UnitOfWork<Product>(sp.GetService<DeliveryDbContext<TCustomer, TItem, THolder>>()));
        services.AddScoped(typeof(IUnitOfWork<ProductImage>), sp => new UnitOfWork<ProductImage>(sp.GetService<DeliveryDbContext<TCustomer, TItem, THolder>>()));
        services.AddScoped(typeof(IUnitOfWork<ProductsInStock>), sp => new UnitOfWork<ProductsInStock>(sp.GetService<DeliveryDbContext<TCustomer, TItem, THolder>>()));
        services.AddScoped(typeof(IUnitOfWork<Order<TItem>>), sp => new UnitOfWork<Order<TItem>>(sp.GetService<DeliveryDbContext<TCustomer, TItem, THolder>>()));
        services.AddScoped(typeof(IUnitOfWork<TItem>), sp => new UnitOfWork<TItem>(sp.GetService<DeliveryDbContext<TCustomer, TItem, THolder>>()));
        services.AddScoped(typeof(IUnitOfWork<ProductCustomer<TItem>>), sp => new UnitOfWork<ProductCustomer<TItem>>(sp.GetService<DeliveryDbContext<TCustomer, TItem, THolder>>()));
