...
public static class TypeExtensions
{
    ...
    /// Поиск типа по имени
    /// </summary>
    public static bool IsExtends(this Type text, string name)
    {
         
        return TypeUtils.IsExtendedFrom(text,name);
    }

    public static Dictionary<string, string> GetAttributes(this Type type)
    {
        return AttrsUtils.GetEntityContrainsts(type);
    }
    public static T FromAttributes<T>(this Type type)
    {
        var target = FactoryUtils.Get().CreateWithDefaultConstructor<T>(typeof(T));
        var dictionary = AttrsUtils.GetEntityContrainsts(type);
        foreach (var p in target.GetType().GetProperties())
        {
            if(dictionary.ContainsKey(p.Name + "Attribute"))
            {
                Setter.SetValue(target, p.Name, dictionary[p.Name + "Attribute"]);
            }
            else if(dictionary.ContainsKey(p.Name + "Attribute"))
            {
                Setter.SetValue(target, p.Name, dictionary[p.Name]);
            }
