...
public class Utils
{
    ...
    {
        throw new NotImplementedException($"{typeof(Utils).GetTypeName()}");
    }
    ...
    {
        string typeName = typeof(T).GetNameOfType();
    ...
    public static Dictionary<string, string> ForMethod(Type controllerType, string name)
    {
        ...
        controllerType.Info($"{controllerType.GetNameOfType()}ForMethod({name})=>\n{attrs.ToJsonOnScreen()}");
        return attrs;
    }

    public static Dictionary<string, string> ForMethodParam(Type controllerType, string action, string name)
    {
        Dictionary<string, string> attrs = new Dictionary<string, string>();
        var method = controllerType.GetMethods().FirstOrDefault(m => m.Name == action);
        if (method == null)
            throw new ArgumentException("name");

        
        var par = method.GetParameters().FirstOrDefault(par => par.Name == name);
        foreach (var data in par.GetCustomAttributesData())
        {
            string key = data.AttributeType.Name;
            foreach (var arg in data.ConstructorArguments)
            {
    ...
    {
        var attrs = ForMethod(type, action);
        return attrs.ContainsKey(nameof(LabelAttribute)) ? attrs[nameof(LabelAttribute)] : type.GetNameOfType();
    ...
    {
        if(model.GetProperties().Any(prop => prop.Name == name))
        {
        ...
        else if (model.GetMethods().Any(prop => prop.Name == name))
        {
            Dictionary<string, string> attrs = ForMethod(model, name);
            return attrs.ContainsKey(nameof(LabelAttribute)) ? attrs[nameof(LabelAttribute)] :
                attrs.ContainsKey(nameof(DisplayAttribute)) ? attrs[nameof(DisplayAttribute)] : name;
        }
        else
    ...
    {
        if (attrs.ContainsKey(nameof(NotInputAttribute)))
            return false;
        if (GetInputType(attrs) == null)
            return false;
        return true;
    ...
    public static string GetAttribute<TAttribute>( Type ptype, string defaultValue ) where TAttribute: Attribute
    {
        var attrs = ForType(ptype);
        string fullname = typeof(TAttribute).GetTypeName();
        string shortname = typeof(TAttribute).GetTypeName().Replace("Attribute","");
        if (attrs.ContainsKey(fullname))
            return attrs[fullname];
        else if (attrs.ContainsKey(shortname))
    ...
    {
        Type ptype = p is Type? (Type)p: p.GetType();
        string result = GetAttribute<DescriptionAttribute>(ptype, "");
        if(String.IsNullOrWhiteSpace(result))
            result = GetAttribute<DescriptionAttribute>(ptype, "");
        return result;
    }
    ...
    public static Dictionary<string, string> ForProperty(Type type, String property)
    {
        ...
        if (info == null)
        {
            Api.Utils.Error($"Свойство {property} не найдено в обьекте типа {type.Name}");
            throw new Exception($"Свойство {property} не найдено в обьекте типа {type.Name}");
        }
        foreach (var data in info.GetCustomAttributesData())
        {
