...
public class ViewItem : StyledItem, IUserAgent
{
    public System.Action<object> OnClick { get; set; } = (target) => { };
    ...
    [Help("Форматы представления представляют из себя классы CSS стилей")]
   ...
   /* {
    ...
    [JsonIgnore()]
    public IUserAgent Client { get; set; }
    ...
    public ViewItem():base()
    {
        ...
        {
            Api.Utils.Info("Children changed");
        };*/
    }
    ...
    {
        string itemTypeName = TypeUtils.ParseCollectionType(GetType().GetProperty(propertyName).PropertyType);
        ...
        foreach(var jvalue in dataset)
        {
            ...
            {
                object value = ObjectCompileExpExtensions.Expression.Compile(p.Value, jvalue);
