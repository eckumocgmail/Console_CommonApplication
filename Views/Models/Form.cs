...
public class Form : PanelItem
{
    ...
    public void AddFormField(string name, Type parameterType, Dictionary<string, string> attrs)
    {
    ...
    public Form(object item) : this()
    {
        ...
        IsValid = false;
        Item = item;
        Title = Chapter = AttrsUtils.LabelFor(item.GetType());
        Description = Chapter = AttrsUtils.DescriptionFor(item.GetType());
        if (item == null)
        {
            NullValue = true;
        }
        ...
        Api.Utils.Info($"{GetType().Name} Constructed");
    }
    ...
    public Form(object item, params string[] propertyNames) : this()
    {
        ...
        StructuresAreEditable = false;
        Item = item;
        Chapter = AttrsUtils.LabelFor(item.GetType());
        if (item == null)
        {
            NullValue = true;
        };
    ...
    public object Update(string[] propertyNames)
    {
        ...
        form.Item = item;
        form.Title = form.Title == null ? AttrsUtils.LabelFor(item) : form.Title;
        form.Description = AttrsUtils.DescriptionFor(item);
        ...
        form.FormFields.ForEach((f) =>
        {
            ...
            ((FormField)f).Edited = form.Edited ? true : false;
            ((FormField)f).Editable = form.Editable ? true : false;
            ((FormField)f).Selectable = form.Selectable ? true : false;
            ((FormField)f).Focusable = form.Focusable ? true : false;
            ((FormField)f).Checkable = form.Checkable ? true : false;
            ((FormField)f).Draggable = form.Draggable ? true : false;
        });
    ...
    public List<object> CreateFormFields(Type type, string[] propertyNames, object target = null)
    {
        ...
        foreach (var propertyName in propertyNames)
        {
            try
            {
                ...
                var Attributes = AttrsUtils.ForProperty(type, property.Name);

                global::FormField field = new global::FormField() { };
                field.Attributes = Attributes;
                field.Name = property.Name;
                field.Label = AttrsUtils.LabelFor(type, property.Name);
                field.Description = AttrsUtils.DescriptionFor(type, property.Name);
                field.Icon = AttrsUtils.IconFor(type, property.Name);
                field.Visible = property.Name == "ID" ? false : AttrsUtils.IsVisible(type, property.Name);
                field.Help = AttrsUtils.HelpFor(type, property.Name);
                ...
                {
                    bool isInput = AttrsUtils.IsInput(type, property.Name);
                    if (isInput == false)
                    {
                        continue;
                    }
                    field.Type = FormControlBuilderUtils.GetControlType(type, property.Name);
                    if (field.Type != null)
                    {
                        ...
                        field.Control = (ViewItem)attribute.CreateControl(field);
            ...
            {
                this.Error("Ошибка при создании поля ввода для свойства " + propertyName,ex);                
            }
        }
        return fields;
    }
    ...
    {
        string result = AttrsUtils.GetInputType(type, property);
    ...
    public List<string> ValidateFormField(FormField field)
    {
    ...
    public override Dictionary<string, List<string>> Validate()
    {
        ...
        else
        {
            if (Typing.ReferenceIsDictionary(Item))
            {
                ...
                {
                    List<string> errors = ValidateFormField((FormField)field);
    ...
    {
        return (FormField)(from p in FormFields where ReflectionService.GetValueFor(p, "Name").ToString() == propertyName select p).FirstOrDefault();
    }
