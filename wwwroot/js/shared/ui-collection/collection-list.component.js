...
class ListItem extends CommonComponent {
    constructor(data) {
        super();
        this.label = null;
        this.data = data;
...
let CollectionListComponent = class CollectionListComponent extends ComponentPrototype {
...
CollectionListComponent = __decorate([
    ...
    <div style="padding: 20px;">
        ...
        <md-list ng-cloak> 
            <md-list-item ng-repeat="listitem in dataset" 
                          style="word-break: break-all;"
                          ng-hide="listitem.hidden"
                          class="noright">
                <img alt="{{ listitem.data[binding['label']] }}" 
                        ng-src="{{ listitem.data[binding['image']] }}" 
                        ng-if="binding && binding['image'] && listitem.data[binding['image']]"
                        class="md-avatar" />
                <md-checkbox class="md-primary" 
                             ng-if="binding && binding['image']"
                             ng-model="listitem.data[binding['checked']]"
                             ng-change="append(listitem)">
                </md-checkbox>
                <p>{{ listitem.label }}</p>

                <i ng-if="binding && binding['icon']"
                    class="material-icons md-secondary md-hue-3" >
                    {{ listitem.data[binding['icon']] }}
                </i>
