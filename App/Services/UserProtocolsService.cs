...
public class UserProtocolsService
{
    protected AuthorizationDataModel _context;

    public UserProtocolsService( AuthorizationDataModel context )
    {
        _context = context;
    }
    ...
    public string ToSql(MessageProtocol model){
        return ( ToSql(CreateDataModel(model)));
    }

    private string ToSql(TableMetaData tableMetaData)
    {
        throw new NotImplementedException($"{GetType().GetTypeName()}");
    }

    private TableMetaData CreateDataModel(MessageProtocol model)
    {
        throw new NotImplementedException($"{GetType().GetTypeName()}");
    }
    ...
    public Form ToForm(MessageProtocol protocol){
        ...
        form.Title = AttrsUtils.LabelFor(protocol.GetLabel());
        form.Description = AttrsUtils.DescriptionFor(protocol.GetDescription());
    ...
    {
        MessageProtocol protocol = (from p in _context.MessageProtocols where p.ID==id select p).FirstOrDefault();
