...
public class SelectControlAttribute: ControlAttribute
{
    ...
    public string GetInputTypes()
    {
        ...
        foreach (string t in TypeUtils.INPUT_TYPES)
        {
            s += $"{t},";
        }
    ...
    {
        string s = "";
        foreach(string t in TypeUtils.PRIMITIVE_TYPES)
        {
            s += $"{t},";
        }
        return s.Length == 0 ? "" : s.Substring(0, s.Length - 1);
    ...
    public override ViewItem CreateControl(object field )
    {
        ...
        string interrpolationValue = ObjectCompileExpExtensions.Expression.Interpolate(_expression, this);
...
{
    public Dictionary<object, object> _options;
