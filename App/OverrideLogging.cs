...
/// </summary>
public class OverrideLogging : LoggerFactory
{
    public static ILogger<T> GetLogger<T>() => Create().CreateLogger<T>();
    public static ILogger GetLogger(string name) => Create().CreateLogger(name);
}
...
/// <summary>
/// Реализует интерфейс логгера
/// </summary>
public class ConsoleLogger<T> : ConsoleLogger, ILogger<T>
{
    
    public ConsoleLogger() : base(typeof(T).GetNameOfType())
    {
    }
}
